...
import { Camera, CalendarDays, CheckCircle2, Send, ChevronLeft, ChevronRight, History, Lock, TrendingUp, TrendingDown, Minus, Sparkles } from "lucide-react"
...
import { format, subDays, addDays, startOfWeek, endOfWeek, eachDayOfInterval, isSameDay, parse } from "date-fns"
...
export default function MoodScannerPage() {
  ...
  const handleHybridComplete = useCallback((moodId: string, quizScore: number, expressionScore: number, totalScore: number) => {
    setSelectedMood(moodId)
    setQuickScanDone(true)
    setShowManualPicker(false)
  }, [])
    ...
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <GlassCard glow>
          ...
          {!quickScanDone && !showManualPicker && (
            <>
              <div className="mb-4">
                <p className="text-xs text-white/50 mb-2">Choose a method:</p>
              </div>
          ...
          {(quickScanDone || showManualPicker) && (
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <p className="text-xs text-white/50 flex items-center gap-1 mb-3">
                ...
                {showManualPicker ? "No camera needed — pick manually" : "Mood detected"}
              </p>
              <div className="flex gap-2">
                {!showManualPicker && (
                  <Button variant="ghost" size="sm" icon={<Camera className="w-4 h-4" />} onClick={() => { setQuickScanDone(false); setSelectedMood(null) }}>
                    Re-scan
                  </Button>
                )}
                <Button variant="ghost" size="sm" icon={<Sparkles className="w-4 h-4" />} onClick={() => { setQuickScanDone(false); setSelectedMood(null); setShowManualPicker(false) }}>
